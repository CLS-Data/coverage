...
clean$country <- df_file %>%
  filter(str_detect(var_low, "(n[0-8]region|nd(9re|10r)gn)")) %>%
  load_files() %>%
  mutate(across(-ncdsid, negative_to_na),
         n6region = values_to_na(n6region, 11)) %>%
  pivot_longer(-ncdsid) %>%
  mutate(sweep = str_extract(name, "\\d+") %>%
           as.integer(),
         sweep = ifelse(sweep <= 6, sweep + 1, sweep + 2),
         fup = fup_dict$ncds[sweep],
         country = case_when(
           sweep <= 4 & between(value, 1, 9) ~ 1,
           sweep <= 4 & between(value, 10, 11) ~ value - 8,
           between(sweep, 5, 11) & between(value, 1, 8) ~ 1,
           between(sweep, 5, 11) & between(value, 9, 10) ~ value - 7,
           sweep == 12 & between(value, 1, 9) ~ 1,
           sweep == 12 & between(value, 10, 11) ~ value - 8,
         ) %>%
...
### Region ----
clean$region <- df_file %>%
  filter(str_detect(var_low, "(n[0-8](region|gor)|nd9(regn|gor)|nd10rgn)")) %>%
  load_files() %>%
  rename(n9region = nd9regn,
         n9gor = nd9gor,
         n10gor = nd10rgn) %>%
  pivot_longer(-ncdsid,
               names_to = c("sweep", ".value"),
               names_pattern = "n(.*)(region|gor)") %>%
  mutate(sweep = as.integer(sweep), 
         sweep = ifelse(sweep <= 6, sweep + 1, sweep + 2),
         fup = fup_dict$ncds[sweep],
         across(c(region, gor), ~ negative_to_na(.x) %>% as_factor()),
         region = if_else(as.integer(gor) %in% 1:12, gor, NA) %>%
           fct_drop()) %>%
  select(ncdsid, fup, region) %>%
  drop_na()
...
clean$marstat <- df_file %>%
  filter(str_detect(variable, "^(n5113|n506515|ms|marital|nd7ms|ND[8-9]MS|nd10ms)$")) %>%
  load_files() %>%
  mutate(
    ...
    marstat_62 = case_when(nd10ms == 1 ~ 3,
                           nd10ms == 2 ~ 2,
                           nd10ms %in% 3:4 ~ nd10ms + 1,
                           nd10ms == 5 ~ 2,
                           nd10ms %in% 6:7 ~ nd10ms - 2,
                           nd10ms == 8 ~ 1)
...
clean$disabled <- df_file %>%
  filter(str_detect(variable, "^(n281|n2662|n4139|n5740|n504121|n504122|n504125|n504128|n504131|dmdisab|bdmph|lsireg2|n7khldsl|ND8KHLDS|ND9DISEQ|nd10diseq)$"),
         dta != "ncds8_imagine") %>%
  load_files() %>%
  mutate(
    ...
    disabled_62 = ifelse(nd10diseq %in% 0:1, nd10diseq + 1, NA),
...
clean$social_class <- df_file %>%
  filter(str_detect(variable, "^(nd7ns8|N8NS8|N9CNS8|nd10nssec5)$")) %>%
  load_files() %>%
  mutate(
    ...
    nssec_62 = ifelse(nd10nssec5 %in% 1:5, nd10nssec5, NA),
...
clean$social_class <- df_file %>%
  filter(str_detect(variable, "^(n236|n660|n190|n1687|n2384|n2393|n6149|n540033|sc|n7sc|N8SC|N9CSC|n10cjsc_dv)$"),) %>%
  load_files() %>%
  mutate(
    across(c(n236, n660), 
    ...
    rgsc_62 = case_when(n10cjsc_dv %in% 1:2 ~ n10cjsc_dv,
                        n10cjsc_dv == 3.1 ~ 3,
                        n10cjsc_dv == 3.2 ~ 4,
                        n10cjsc_dv %in% 4:5 ~ n10cjsc_dv + 1),
