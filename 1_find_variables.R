...
fup_dict <- list(mcs = c(0, 3, 5, 7, 11, 14, 17),
                 next_steps = c(14:20, 25, 31),
                 bcs70 = c(0, 5, 10, 16, 26, 29, 34, 38, 42, 46, 51),
                 ncds = c(0, 7, 11, 16, 23, 33, 42, 44, 46, 50, 55, 62))
...
## d. Unit Tests ----
unit_tests <- function(cleaned, fups){
  expect_length(cleaned$response %>%
                  distinct(fup) %>%
                  pull(fup),
                length(fups))
  
  expect_length(cleaned$wide %>%
                  count(fup) %>%
                  distinct(n) %>%
                  pull(n),
                1)
  
  expect_equal(cleaned$wide %>%
                 distinct(fup) %>%
                 pull(fup),
               cleaned$response %>%
                 distinct(fup) %>%
...
save(list = setdiff(ls(), "df_file"), file = "Data/helpers.Rdata")
