...
clean$ethnicity <- df_file %>%
  filter(str_detect(var_low, "(w[8-9]dethn6|w?ethgrpyp)"),
         str_detect(dta, "(ns[8|9]|young_person)")) %>%
  load_files() %>%
  mutate(
...
## b. Sex ----
clean$sex <- df_file %>%
  filter(str_detect(var_low, "^(w8cmsex|w9dsex|w[1-7]sex(|yp))$"),
         fld == "safeguarded_eul") %>%
  load_files() %>%
...
clean$sexuality <- df_file %>%
  filter(str_detect(var_low, "sexual|w9sori")) %>%
  load_files() %>%
  mutate(across(-nsid,
...
clean$country <- df_file %>%
  filter(variable %in% c("W8DGOR", "W9DRGN", "gor")) %>%
  mutate(sweep = case_when(str_detect(dta, "^wave_two") ~ 2,
                           str_detect(dta, "^wave_three") ~ 3,
                           str_detect(dta, "^ns8") ~ 8,
                           str_detect(dta, "^ns9") ~ 9)) %>%
  arrange(sweep) %>%
  load_files() %>%
  pivot_longer(-nsid) %>%
  mutate(sweep = case_when(name == "gor.x" ~ 2,
                           name == "gor.y" ~ 3,
                           name == "w8dgor" ~ 8,
                           name == "w9drgn" ~ 9),
         fup = fup_dict$next_steps[sweep],
         country = case_when(value %in% 1:9 ~ 1,
                             value %in% 10:12 ~ value - 8) %>%
           fct_from_dict(country_dict)) %>%
  select(nsid, fup, country) %>%
  drop_na()

clean$country <- glue("{flds$next_steps}/safeguarded_eul/ns9_2022_longitudinal_file.dta") %>%
  read_dta() %>%
  rename_with(str_to_lower) %>%
  select(nsid, matches("^w\\doutcome")) %>%
  pivot_longer(-nsid,
               names_to = "sweep",
               names_pattern = "w(\\d)outcome",
               names_transform = list(sweep = as.integer),
               values_to = "outcome") %>%
  filter(outcome == 1,
         sweep == 1) %>%
  mutate(fup = fup_dict$next_steps[sweep],
         country = factor("England", country_dict)) %>%
  select(nsid, fup, country) %>%
...
clean$region <- df_file %>%
  ...
  arrange(sweep) %>%
  load_files() %>%
  mutate(across(-nsid, ~ negative_to_na(.x) %>% values_to_na(13)),
         across(-nsid, ~ factor(.x, 1:12, names(val_labels(w8dgor))))) %>%
  pivot_longer(-nsid, values_to = "region") %>%
  mutate(sweep = case_when(name == "gor.x" ~ 2,
                           name == "gor.y" ~ 3,
                           name == "w8dgor" ~ 8,
                           name == "w9drgn" ~ 9),
         fup = fup_dict$next_steps[sweep]) %>%
  select(nsid, fup, region) %>%
  drop_na()
...
clean$marstat <- df_file %>%
  ...
  mutate(marstat_19 = ifelse(w6marstatyp %in% 1:5, w6marstatyp, NA),
         ...
         marstat_31 = case_when(w9dmarstat == 1 ~ 1,
...
clean$disabled <- df_file %>%
  filter(str_detect(var_low, "w\\ddisab|w[8-9]ddisea"),
         str_detect(dta, "young_person|ns[8-9]")) %>%
  load_files() %>%
  mutate(
    across(c(w8ddisea, w9ddisea), 
           ~ ifelse(.x %in% 0:1, .x + 1, NA),
...
df_xwave <- glue("{flds$next_steps}/safeguarded_eul/ns9_2022_longitudinal_file.dta") %>%
  read_dta() %>%
  rename_with(str_to_lower) %>%
  select(nsid, matches("^w\\doutcome")) %>%
...
unit_tests(next_steps, fup_dict$next_steps)
