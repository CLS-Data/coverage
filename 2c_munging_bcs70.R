...
clean$ethnicity <- df_file %>%
  filter(str_detect(variable, "^(a12_(1|4|7)|c6_14|oa3_1|ethnic|bd7ethnic|e245|bd11ethn6)$"),
  ...
  mutate(
    ...
    ethnic_group_51 = negative_to_na(bd11ethn6),
...
clean$country <- df_file %>%
  ...
  mutate(sweep = str_sub(name, 3, -6) %>%
         ...
         country = case_when(sweep %in% 1:9 ~ as.integer(value),
                             sweep %in% 10:11 ~ c(1, 4, 3, 2)[value]) %>%
           fct_from_dict(country_dict)) %>%
  distinct(bcsid, fup, country) %>%
  drop_na() %>%
  group_by(bcsid, fup) %>%
  filter(n() == 1) %>%
  ungroup()
...
clean$region <- df_file %>%
  filter(str_detect(variable, "BD\\d(|\\d)(REGN|GOR)|bd11rgn")) %>%
  load_files() %>%
  pivot_longer(-bcsid,
               names_to = c("sweep", ".value"),
               names_pattern = "bd(.*)(rgn|regn|gor)") %>%
  mutate(
    sweep = as.integer(sweep),
         fup = fup_dict$bcs70[sweep],
         
         gor = if_else(as.integer(gor) %in% 1:12, gor, NA) %>%
           as_factor() %>%
           fct_recode("Yorkshire and the Humber" = "Yorkshire and Humberside") %>%
           fct_drop(),
         rgn = if_else(as.integer(rgn) %in% 1:12, rgn, NA) %>%
           as_factor() %>%
           fct_recode("Yorkshire and the Humber" = "Yorkshire and The Humber",
                      "Wales" = "(pseudo) Wales",
                      "Scotland" = "(pseudo) Scotland") %>%
           fct_drop(),
         region = coalesce(gor, rgn)
         ) %>%
  distinct(bcsid, fup, region) %>%
  drop_na() %>%
  group_by(bcsid, fup) %>%
  filter(n() == 1) %>%
  ungroup()
...
clean$marstat <- df_file %>%
  filter(str_detect(var_low, "^(b960322|marstat2|bd(\\d|\\d\\d)ms)$")) %>%
  load_files() %>%
  mutate(
    ...
    marstat_42 = clean_bdddms(bd9ms),
    marstat_46 = clean_bdddms(bd10ms),
    marstat_51 = clean_bdddms(bd11ms),
  ) %>%
...
clean$social_class <- df_file %>%
  filter(str_detect(variable, "^(bd7ns8|b8nssec|B9CNS8|BD10CNS8|b11cjnss8)$")) %>%
  load_files() %>%
  mutate(
    nssec_34 = clean_nssec(bd7ns8),
    
    nssec_42 = clean_nssec(b9cns8),
    
    nssec_46 = clean_nssec(bd10cns8),
    
    nssec_51 = clean_nssec(b11cjnss8),
...
df_xwave <- glue("{flds$bcs70}/xwave/bcs70_response_1970-2021.dta") %>%
  read_dta() %>%
  rename_with(str_to_lower) %>%
...
unit_tests(bcs70, fup_dict$bcs70)
