...
clean$region <- df_file %>%
  filter(str_detect(var_low, "^.aregn00$"),
         str_detect(dta, "family_derived"))  %$%
  map2(file_path, variable, load_mcs)  %>%
  reduce(full_join, by = "mcsid") %>%
  pivot_longer(-mcsid) %>%
  mutate(sweep = str_sub(name, 1, 1) %>% match(letters),
         fup = fup_dict$mcs[sweep],
         region = if_else(between(value, 1, 12), value, NA) %>%
           as_factor() %>%
           fct_drop()) %>%
  select(mcsid, fup, region) %>%
  inner_join(df_cm %>% select(-iid), by = "mcsid") %>%
  relocate(cnum, .after = fup) %>%
  drop_na()

clean$region <- df_file %>%
  filter(var_low == "garegn00") %$%
  load_mcs(file_path) %>%
  filter(gcnum00 %in% 1:2) %>%
  select(mcsid, gcnum00, garegn00) %>%
  mutate(fup = 17,
         cnum = as.integer(gcnum00),
         region = if_else(between(garegn00, 1, 12), garegn00, NA) %>%
           as_factor() %>%
           fct_drop()) %>%
  select(mcsid, cnum, fup, region) %>%
  drop_na() %>%
  bind_rows(clean$region)
...
clean$disabled <- df_file %>%
  ...
  drop_na() %>%
  arrange(mcsid, cnum, fup, disabled) %>%
  group_by(mcsid, cnum, fup) %>%
  filter(row_number() == n()) %>%
  ungroup()
